...
*  Block elements have :css:`display: block`;
*  Inline elements have :css:`display: inline`;
...
- :html:`<div>`
- :html:`<h1>` to :html:`<h6>` (headings)
- :html:`<p>` (paragraph)
- :html:`<ul>` and :html:`<ol>` (lists)
- :html:`<li>` (list items)
...
The :html:`<div>` (division) element is a block-level element used as a container to 
group other HTML elements. It doesn't have any special formatting or meaning, rather it 
they are used for:

*  **Layout:** A div can be used to create sections like headers, footers, sidebars, 
   and main content areas.
*  **Styling Sections:** It can wrap multiple elements to apply styles or manipulate 
   them as a single unit.

:html:`<div>` elements are :term:`block` elements and we will use them in the following
section to explain the :term:`box model`.

**Example:**

In this example, the :html:`<div>` groups the heading and paragraph together, allowing 
you to style them as a single section.
...
The CSS Box Model is a way of describing the space a :term:`block` element occupies on 
...
- :html:`<a>` (links)
- :html:`<img>` (images)
- :html:`<b>` (text formatting)

**Example:**

Here, the :html:`<strong>` tag is an inline element that styles the word "bold" without 
...
*  ``inline-block`` - The element is formatted as an inline element but allows setting 
   of properties like height and width.
...
By default, :html:`<span>` is an inline element. You can make it behave like a block 
element using :css:`display: block;`.
...
*  The :html:`<span>` now starts on a new line and takes up the full width available.
*  It pushes the following :html:`<p>` element to the next line.
*  The background color helps visualize the block area.

Example: Make an Inline Element
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

By default, :html:`<div>` is a block element. You can make it behave like an inline 
element using :css:`display: inline;`.
...
.. figure:: img/box_web_inspector.jpg
      ...
      Web inspector view of an earlier example showing the margin of the :html:`<h2>`
