...
of text such as the font used, font size, font colour all the way to complex masking or
setting style dynamically based on whether the cursor is hovering over the element.
...
As shown in the example below the ``<span>`` tag is a generic tag than can be used to
select a section of the document. Here the child ``<span>`` tag overwrites the colour of
the parent ``<p>``. Note that the font size doesn't change because that hasn't been
overwritten.
