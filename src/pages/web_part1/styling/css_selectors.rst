...
Applying styling to individual elements will quickly become tedious when you have a
complicated document. For example, if you need to change the color of headings, you will
need to search through the document to find all headings and replace the colour value.
...
syntax for selectors is the selector followed by a block of CSS declarations enclosed in
curly braces
...
An example to change all paragraphs, ``<p>`` to use a particular font size and color is:
...
In this case the selector is ``p``, which refers to the paragraph element or ``<p>``
tag.

Internal CSS
------------

Internal CSS is written within a ``<style>`` tag inside the ``<head>``` section of an
HTML document and applies styles to the whole page.

You may place as many different selectors inside the ``<style>`` tag as you like.

In the example below styles will be applied to all first level headings and paragraphs.
...
Your CSS might become quite lengthy and hard to manage. To keep your HTML simple you can
define the CSS in a separate file with a ``.css`` extension. To include the styles
defined in this file it is linked using a ``<link>`` tag. You need to specify how the
linked file is **re**\lated to the HTML, in this case we have a stylesheet, which
specifies the formatting. Then we specify the **h**\ypertext **ref**\erence, i.e the
name of the ``.css``.
